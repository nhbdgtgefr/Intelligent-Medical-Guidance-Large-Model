...
def create_slices(image_path, slices_save_dir: Path):
    ...
    wh_ratio = max(width / height, height / width)
    if wh_ratio < ratio_thres:
        return [{"img": image_path, "top_left": (0, 0)}]  # 返回包含字典的列表
...
def ocr_pred(ocr_model, image_path: Path, output_dir: Path, show_res=False):
    ...
    # 如果图像太大，进行切图
    # 创建横切图
    image_slices = create_slices(str(image_path), work_dir)  # 确保这里使用的是正确的变量名

    result = []
    for img_info in image_slices:  # 使用正确的变量名
        print(type(img_info), img_info)  # 这行代码用于调试，查看 img_info 的类型和内容
        
        img_path = img_info["img"]  # 确保这里使用的是正确的键
        # 推理
